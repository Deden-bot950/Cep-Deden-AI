...
└── netlify/functions/
    ...
    ├── start-video-background.js     ← BACKGROUND FUNCTION: proses video Veo (wates 15 menit)
    ├── video-job-status.js           ← cek status job video (dipaké frontend polling)
    ├── jobs.js                       ← modul panyimpen status job video (dipaké fungsi di luhur)
...
**Catetan**: `generate-video.js` jeung `video-status.js` (versi heubeul) geus **teu dipaké deui** — diganti ku `start-video-background.js` + `video-job-status.js` sabab fungsi Netlify biasa ngan méré wates 10 detik, teu cukup keur Veo. Lamun aya di GitHub, mangga dihapus supados teu numpuk/matak bingung.
...
## Setup video (Background Function)

Video pakéna arsitéktur béda ti chat/gambar, sabab Veo butuh waktos leuwih ti 10 detik. Prosésna:
1. Frontend manggil `start-video-background` (Netlify Background Function, wates 15 menit) sarta langsung diteruskeun (teu ditungguan)
2. Frontend polling `video-job-status` unggal 10 detik nepi ka status-na `done` atawa `error`
3. Hasilna disimpen samentawis di bin JSONBin misah (`JSONBIN_JOBS_BIN_ID`)

**Léngkah setup tambahan (sakali wungkul):**
1. Buka jsonbin.io → jieun bin anyar, eusina persis: `{"jobs":{}}`
2. Copy Bin ID-na
3. Tambahkeun env variable di Netlify: `JSONBIN_JOBS_BIN_ID` = ID éta

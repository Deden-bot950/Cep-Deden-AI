...
// Body: { provider, model, messages, systemPrompt, email }
// Wates kuota dicek langsung di dieu (server-side) ngaliwatan quota.js.
...
exports.handler = async function (event) {
  ...
  const { provider, model, messages, systemPrompt, email } = payload;

  try {
    const quota = await checkAndIncrement(email, 'chat');
    if (!quota.allowed) {
      return {
        statusCode: 429,
        body: JSON.stringify({ error: `Wates ${quota.limit} chat/poé geus kapaké sadayana. Cobian deui isuk, nya.` }),
      };
    }

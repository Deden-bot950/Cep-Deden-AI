...
// Body: { prompt, referenceImage?, email }
...
exports.handler = async function (event) {
  ...
  const { prompt, referenceImage, email } = payload;
  if (!prompt) {
    return { statusCode: 400, body: JSON.stringify({ error: "prompt wajib diisi" }) };
  }

  try {
    const quota = await checkAndIncrement(email, 'image');
    if (!quota.allowed) {
      return {
        statusCode: 429,
        body: JSON.stringify({ error: `Wates ${quota.limit} gambar/poé geus kapaké sadayana. Cobian deui isuk, nya.` }),
      };
    }
  } catch (err) {
